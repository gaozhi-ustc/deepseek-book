...
# 第 3 章  工程实现分析
本章将按照第2章介绍的系统结构的创新点，结合DeepSeek已经公开的技术资料，分别介绍基础设施、MoE、MLA和强化学习等几个技术方向的工程实现。此外，随 DeepSeek-V4 发布的新一代开源训练框架 DSpark 将在 3.5 节介绍（该节内容尚在撰写中，当前仅列出大纲）。
...
## 3.5 DSpark：DeepSeek-V4 开源训练框架

> **【本节大纲占位，内容待撰】** DSpark 是 DeepSeek 随 V4 发布的新一代开源训练框架。本节由 lmxxf 老师撰写中，尚未交稿；以下为暂定大纲，待稿件到位后替换，具体小节划分以正式稿为准。

### 3.5.1 DSpark 框架定位与总体架构（待撰）

DSpark 在 DeepSeek 开源工程体系中的定位，与第2章 V4 架构创新（CSA/HCA、mHC、Muon、FP4 QAT）的对应关系，以及框架的整体分层设计。

### 3.5.2 CSA/HCA 稀疏注意力的算子实现（待撰）

Compressor 加权池化、Lightning Indexer top-k 选择与滑动窗口分支的 kernel 级实现，以及压缩 KV cache 的存储布局与混合精度（FP8/FP4）处理。

### 3.5.3 mHC 多路残差的高效实现（待撰）

4 路残差流的显存管理与激活重计算策略，Sinkhorn-Knopp 迭代（T=20）的 FP32 混合精度实现与独立 CUDA Stream 调度。

### 3.5.4 Muon 优化器的分布式实现（待撰）

Hybrid Newton-Schulz 迭代（8+2 步）在大规模并行下的实现，Muon/AdamW 参数分组在分布式优化器状态切分中的工程处理。

### 3.5.5 FP4 量化感知训练与推理支持（待撰）

FP4 QAT 的直通估计器实现、FP4 到 FP8 的无损反量化路径，以及与硬件 Tensor Core 的协同。

### 3.5.6 与既有开源组件的协同（待撰）

DSpark 与 DeepGEMM、DeepEP、FlashMLA、EPLB 等第 3.1～3.3 节所述组件的集成关系与演进对比。
