...
本章系统剖析 DeepSeek 模型的核心技术原理。2.1 节给出 2026 年发布的 DeepSeek V4 的架构全景、训练体系与设计哲学，为全章建立坐标；2.2～2.4 节深入其基础组件的原理——MoE 前馈网络、MLA 与 DSA 注意力机制，以及以纯强化学习（Pure RL）为基础的深度推理能力构建与融合机制；2.5～2.7 节则剖析 V4 面向百万级上下文引入的三大架构创新：CSA/HCA 混合稀疏注意力、mHC 流形约束超连接与 Muon 优化器。
...
V4 的架构组成可以概括为"继承的基座、调整的 MoE、新增的三大创新"三个层面。

#### 2.1.2.1 继承自 V3 的基础组件
...
**MLA（多头潜在注意力，Multi-head Latent Attention）**：通过低秩压缩将 KV 缓存压缩至潜空间，显著降低推理时的 KV 缓存显存占用。V4-Pro 的 query 压缩维度为 1536，输出投影分为 16 组。V4-Flash 对应为 1024 和 8 组。

**MTP（多词元预测，Multi-Token Prediction）**：在训练阶段，模型不仅预测下一个 token，还同时预测后续多个 token（V4 的 MTP 深度为 1，即额外预测 1 个 token）。这提供了更丰富的训练信号，加速收敛。
...
#### 2.1.3.4 Anticipatory Routing（预防性路由）
...
#### 2.1.3.5 SwiGLU Clamping

SwiGLU 激活函数[^swiglu]是 V3/V4 FFN 层的基础组件，定义为：
...
DeepSeek-V3官方技术报告中用图2-1表示了其模型架构，该架构被DeepSeek-R1和V3.1完全沿用，亦是DeepSeek-V3.2的基础，奠定了该系列模型‘高性能与低成本’的核心优势。

![](pic/2-1.png)

图2-1 DeepSeek-V3的基本架构示意图
...
具体而言，DeepSeek-V3[^dsv3] 是一个典型的Transformer架构LLM，总共包括61个Transformer层。在每一层中，信号先经过归一化，然后进入注意力机制层；再次经过归一化，再进入FFN（前馈网络，Feed-Forward Network）。图2-1的上半部分展示了以DeepSeekMoE为核心的FFN结构，下半部分展示了DeepSeek-V3的注意力机制MLA。本节将重点剖析第一部分，而2.3节将对DeepSeek的注意力机制进行详细展开。

### 2.2.1 MoE 架构原理

DeepSeek-V3的61个Transformer层中前3层采用了稠密FFN，其余58层均采用了MoE结构。不同于传统MoE，DeepSeekMoE[^dsmoe] 提出了细粒度专家分割与共享专家机制相结合的方案。本节将首先分析传统 MoE 架构的局限性，随后分别详细阐述 DeepSeekMoE 的细粒度分割原理、共享与路由专家的协同机制，以及具体的路由激活算法实现。
...
MLA 是 DeepSeek-V2[^dsv2] 中首次提出并被 DeepSeek-V3 沿用的核心架构组件。其设计初衷是为了在保证模型具备标准MHA（Multi-Head Attention，多头注意力）强大学习能力的同时，将推理阶段的键-值缓存显存占用降低至极低水平。
...
通过引入 MLA 机制，DeepSeek 将推理过程中的 键-值 缓存显存占用降到了极低水平，彻底打破了长上下文场景下的显存瓶颈（Memory Bottleneck）。然而，解决了‘存得下’的问题后，DeepSeek 又面临着另一个严峻挑战：随着序列长度的不断攀升（如扩展至 128K 甚至更长），全注意力机制固有的平方级计算复杂度使得推理延迟急剧增加。为了在保持低显存优势的同时，进一步攻克长文本推理的计算瓶颈，DeepSeek-V3.2[^dsv32] 在 MLA 的架构基础上，引入了 DSA（DeepSeek Sparse Attention）。
...
以 V4-Pro 的 61 层架构、50000 token 上下文为例，标准多头注意力（MHA）需存储约 81.5 GiB 的 KV cache。此后各类注意力方案的设计，本质上都在回答同一个问题：**如何让 KV cache 更小、更快、更省？**

#### 2.5.1.2 既有技术路线及其瓶颈

围绕这一瓶颈，业界先后给出了多条技术路线，每一条都在解决前一条未解决的问题，如表2-8 所示。
...
**MLA** 开辟了全新维度——不再主要靠砍 KV 头数，而是砍每条 KV 的维度。其核心思想是将 Key 和 Value 联合压缩为一个低维 latent 向量：
...
#### 2.5.1.4 序列维度的稀疏：DSA 与 NSA

**DSA** 首次触及序列维度：推理时用轻量 Indexer 从全部 KV cache 中选出 top-k 条，注意力仅在选中的子集上计算。但 DSA 的两个根本局限是：（1）KV cache 全量存储不变，只省计算不省显存；（2）训练时仍用完整注意力，推理时才切换为稀疏——"训完套眼镜"导致训推不一致。

**NSA**[^nsa]解决了训推一致性问题，提出**原生可训练的稀疏注意力**。其架构为三分支并行：压缩全看（Compressed）、选块精读（Selected）、滑动窗口（Sliding Window），通过学习门控加权合并。NSA 在 27B 模型上验证了"原生稀疏不弱于全注意力"的关键结论，但工程规模仅为实验室级别。
...
4. **输出**：Grouped Output Projection（见 2.5.2.5 节）
...
NSA 原始设计与 V4 落地版本的完整对照如表2-11 所示。

表2-11 NSA 原始设计与 V4 CSA/HCA 的工程对照
...
表2-12 50K 上下文、61 层架构的 KV cache 总量
...
表2-14 HC 与 mHC 信号增益特性对比（27B 模型）
...
#### 2.6.3.4 与 Layer Normalization 的互补关系
...
**动量法**。Polyak 提出的 heavy ball 方法[^polyak]引入动量缓冲区，对历史梯度做指数移动平均：
...
Muon（MomentUm Orthogonalized by Newton-Schulz）由 Keller Jordan 于 2024 年提出[^muon]，先在 NanoGPT speedrun 排行榜上展现优势，随后在 Moonshot Kimi K2（2025）和 DeepSeek V4（2026）等超大规模模型上得到验证。
...
#### 2.7.2.4 Newton-Schulz 迭代：高效近似正交化
...
#### 2.7.3.2 Hybrid Newton-Schulz 迭代细节
...
#### 2.7.3.4 例外：Embedding 为何保留 AdamW
