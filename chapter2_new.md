# 第2章 DeepSeek 核心技术之原理解析

本章将系统剖析 DeepSeek 模型家族的核心架构原理。全章以技术演进为主线，首先阐述DeepSeek LLM效率和智能“双主线”上升的发展路径；随后深入解析 其核心网络架构与注意力机制的设计细节；最后揭示以纯强化学习（Pure RL）为基础的推理能力构建，以及推理能力（深度思考模式）与通用能力（非思考模式）融合的机制。在此基础上，2.1 节还将给出 2026 年发布的 DeepSeek-V4 的架构总览，本章最后三节（2.5～2.7）则深入剖析其面向百万级上下文的三大核心创新：CSA/HCA 混合稀疏注意力、mHC 流形约束超连接与 Muon 优化器。

## 2.1 总体架构演进与 DeepSeek-V4 总览

### 2.1.1 架构演进关键节点

自 DeepSeek-V3 与 R1 模型发布并引发业界轰动以来，DeepSeek 的大语言模型在 2025 年并未如市场预期的那样快速进行大版本号更新，而是围绕 V3/R1 体系持续迭代打磨。这并不意味着 DeepSeek 发展的停滞：如图2-1所示的测评结果，V3.2模型在各项指标上依然保持着SOTA（State-of-the-Art）水平，具备和世界最先进大模型竞争的能力。在此沉淀的基础上，2026 年第二季度 DeepSeek 正式发布了新一代旗舰模型 V4，在注意力机制、残差连接与优化器三个维度引入了代际性的架构创新。

![](pic/2-1.png)

图2-1 DeepSeek-V3.2 及其竞品的基准测试对比

这一时期 DeepSeek 从最初分别致力于通用任务的 V3 和深度逻辑推理的 R1，逐步演化出了融合两种能力的 V3.2 模型，并最终迭代出支持 1M 上下文的 V4 模型，在降低成本方面也有持续创新。本节将系统回顾 DeepSeek 在此周期内的关键架构迭代，分析其如何通过工程创新，在计算效率、推理深度与任务泛化能力之间进行权衡与优化。DeepSeek 2025–2026 年度架构演进关键节点如表2-1所示。


**表 2-1** **DeepSeek 2025–2026 年度架构演进关键节点**
...
<tbody>
<tr>
<td style="text-align: left;">发布
时间</td>
<td style="text-align: left;">版本</td>
<td style="text-align: left;">核心技术创新</td>
<td style="text-align: left;">解决的主要问题</td>
</tr>
<tr>
<td style="text-align: left;">2024年Q4</td>
<td style="text-align: left;">DeepSeek-V3</td>
<td style="text-align: left;">DeepSeekMoE, MLA, MTP</td>
<td style="text-align: left;">超大规模模型训练与运行成本</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q1</td>
<td style="text-align: left;">DeepSeek-R1</td>
<td style="text-align: left;">GRPO, 纯强化学习/多阶段训练</td>
<td style="text-align: left;">深度思考能力的诱导与优化</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q1</td>
<td style="text-align: left;">DeepSeek-V3-0324</td>
<td style="text-align: left;">专项能力优化与指令遵循微调</td>
<td style="text-align: left;">复杂任务（数学、代码、写作）性能提升与 Function Calling 稳定性</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q2</td>
<td style="text-align: left;">DeepSeek-R1-0528</td>
<td style="text-align: left;">思维链深度扩展、推理感知微调、能力蒸馏</td>
<td style="text-align: left;">增强推理深度与准确性、降低幻觉、补齐工具调用短板，并将推理能力有效蒸馏至轻量模型</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q3</td>
<td style="text-align: left;">DeepSeek-V3.1</td>
<td style="text-align: left;">混合推理模式，统一权重</td>
<td style="text-align: left;">通用能力与深度推理能力在单一模型中的整合与灵活切换</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q3</td>
<td style="text-align: left;">DeepSeek-V3.1-Terminus</td>
<td style="text-align: left;">输出质量针对性优化</td>
<td style="text-align: left;">修复中英文混杂、异常字符等输出一致性问题</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q4</td>
<td style="text-align: left;">DeepSeek-V3.2</td>
<td style="text-align: left;">DSA，可扩展混合RL，智能体任务数据合成</td>
<td style="text-align: left;">长上下文计算效率、后训练强化、智能体场景性能</td>
</tr>
<tr>
<td style="text-align: left;">2025年Q4</td>
<td style="text-align: left;">DeepSeek-V3.2-Speciale</td>
<td style="text-align: left;">无界思维链训练</td>
<td style="text-align: left;">探索深度思考模式的潜能，在最高难度推理任务上拓展大模型的性能边界</td>
</tr>
<tr>
<td style="text-align: left;">2026年Q2</td>
<td style="text-align: left;">DeepSeek-V4 (Pro / Flash)</td>
<td style="text-align: left;">CSA+HCA 混合注意力, mHC, Muon 优化器, 1M 上下文</td>
<td style="text-align: left;">百万级超长上下文的计算与显存效率，推理成本经济可行，原生智能体能力</td>
</tr>
</tbody>
</table>

### 2.1.2 DeepSeek-V4 的定位与参数规格

表2-1 所示演进的终点，是 2026 年第二季度发布的 DeepSeek-V4。在深入 V3/R1 体系的技术细节之前，本节先勾勒 V4 的整体轮廓——它革新了什么、继承了什么、如何训练——为 2.5～2.7 节的深入剖析建立全局坐标。

DeepSeek V4 是 DeepSeek 系列大语言模型的第四代架构，延续了 V3 的条件计算（Conditional Computation）路线，在推理能力和长上下文处理能力上实现了代际跨越。V4 提供两个规格：

表2-2 DeepSeek-V4 模型参数规格
...
|------|---------|---------|--------------|---------|------|
| CSA/HCA 混合稀疏注意力 | 注意力机制 | 全注意力 | 1M 上下文下 $O(n^2)$ 计算与 KV cache 存储不可行 | 相比 V3.2，V4-Pro 单 token 推理 FLOPs 约 27%、KV cache 约 10% | 2.5 节 |
...
简要而言：CSA（Compressed Sparse Attention）将 KV cache 沿序列维度以压缩比 $m=4$ 加权池化，再由 Lightning Indexer 精选 top-$k$ 条参与注意力计算，负责"从全局历史中精选细节"；HCA（Heavily Compressed Attention）以 $m'=128$ 重度压缩后全部参与计算，负责"以极低成本扫读全局大意"；两类注意力层交替堆叠，且均配有 128 条滑动窗口分支保留最近上下文的原始精度。mHC 将残差流扩展为 4 条并行路径，并把路径间的混合矩阵经 Sinkhorn-Knopp 迭代投影到双随机矩阵流形（Birkhoff 多面体），以谱范数有界性从数学上保证任意深度下信号增益不发散。Muon 则对动量矩阵做极分解意义上的正交化——保留更新方向、拉平各方向强度；Embedding、输出头、RMSNorm 及 mHC 静态偏置等少数参数仍保留 AdamW。
...
### 2.1.5 MoE 参数演进：从 V3 到 V4

V4 对 MoE 层进行了多项重要调整，核心方向是"更多专家、更稀疏激活、更稳定路由"。

表2-4 DeepSeek MoE 参数变化（V3 vs V4-Pro vs V4-Flash）
...
#### 2.1.6.4 Anticipatory Routing（预防性路由）
...
本章后续将对上述两代架构的核心组件分别进行深入剖析：2.2～2.4 节聚焦 V3/R1 体系的 DeepSeekMoE、MLA、DSA 与 GRPO 等技术；2.5～2.7 节则深入 V4 的三大架构创新——CSA/HCA 混合稀疏注意力、mHC 流形约束超连接与 Muon 优化器。

## 2.2 前馈网络架构（核心网络架构）

DeepSeek-V3官方技术报告中用图2-2表示了其模型架构，该架构被DeepSeek-R1和V3.1完全沿用，亦是DeepSeek-V3.2的基础，奠定了该系列模型‘高性能与低成本’的核心优势。

![](pic/2-2.png)

图2-2 DeepSeek-V3的基本架构示意图
...
具体而言，DeepSeek-V3是一个典型的Transformer架构LLM，总共包括61个Transformer层。在每一层中，信号先经过归一化，然后进入注意力机制层；再次经过归一化，再进入FFN（前馈网络，Feed-Forward Network）。图2-2的上半部分展示了以DeepSeekMoE为核心的FFN结构，下半部分展示了DeepSeek-V3的注意力机制MLA。本节将重点剖析第一部分，而2.3节将对DeepSeek的注意力机制进行详细展开。

### 2.2.1 MoE架构详解

DeepSeek-V3的61个Transformer层中前3层采用了稠密FFN，其余58层均采用了MoE结构。不同于传统MoE，DeepSeekMoE提出了细粒度专家分割与共享专家机制相结合的方案。本节将首先分析传统 MoE 架构的局限性，随后分别详细阐述 DeepSeekMoE 的细粒度分割原理、共享与路由专家的协同机制，以及具体的路由激活算法实现。
...
从数学结构上看，假设一个标准的 MoE 架构（如 GShard）包含 $\text{N}$ 个专家，每个专家的前馈网络（FFN）中间隐藏层维度为 $\text{d}$，每次推理时针对每个 Token 激活前 $\text{K}$ 个专家。细粒度分割策略引入了一个超参数 $\text{m}$（分割因子），将每个原始专家在隐藏层维度上分割为 $\text{m}$ 个更小的子专家。

经过分割，模型的架构发生如下演变：

- **专家总数**：从 $\text{N}$ 增加至 $\text{mN}$。

- **单个专家维度**：中间隐藏层维度从 $\text{d}$ 缩减为 $\text{d}\text{/}\text{m}$。

- **激活专家数**：为了维持计算开销不变，每次激活的专家数量从 $\text{K}$ 调整为 $\text{mK}$。
...
在数学形式上，设 $\text{u}_{\text{t}}$ 为第 $\text{t}$ 个 Token 的输入向量，DeepSeekMoE 层的输出 ${\text{h}^{\text{'}}}_{\text{t}}$ 计算如下：

$$
{\text{h}^{\text{'}}}_{\text{t}}\text{=}\text{u}_{\text{t}}\text{+}\sum_{\text{i}\text{=1}}^{\text{N}_{\text{s}}}\text{FFN}_{\text{i}}^{\text{(}\text{s}\text{)}}\text{(}\text{u}_{\text{t}}\text{)+}\sum_{\text{j}\text{=1}}^{\text{N}_{\text{r}}}\text{g}_{\text{j}\text{,}\text{t}}\text{FFN}_{\text{j}}^{\text{(}\text{r}\text{)}}\text{(}\text{u}_{\text{t}}\text{)}
$$

其中：

- $\text{N}_{\text{s}}$表示共享专家的数量，$\text{FFN}_{\text{i}}^{\text{(}\text{s}\text{)}}$ 为第 $\text{i}$ 个共享专家，在DeepSeek-V3中，对于每个MoE层，$\text{N}_{\text{s}}\text{=1}$。

- $\text{N}_{\text{r}}$表示路由专家的总数，$\text{FFN}_{\text{j}}^{\text{(}\text{r}\text{)}}$ 为第 $\text{j}$ 个路由专家，在DeepSeek-V3中，对于每个MoE层，$\text{N}_{\text{r}}\text{=256}$。

- $\text{g}_{\text{j}\text{,}\text{t}}$ 为路由专家 $\text{j}$ 对 Token $\text{t}$ 的门控权重，仅对被选中的 Top-$\text{K}$ 个专家非零，在DeepSeek-V3中，对于每个MoE层，$\text{K}\text{=8}$，关于$\text{g}_{\text{j}\text{,}\text{t}}$的计算将在下文介绍。

根据DeepSeek的实验结果，在传统MoE架构模型的基础上仅仅增加一个共享专家，就能在大多数基准上提升性能；相反，如果禁用共享专家，即便额外激活一个路由专家来弥补计算量，模型的性能也会受到显著影响。图2-3给出了消融实验结果。
...
图2-3的消融实验结果表明：相较于传统架构，共享专家与路由专家的分工显著降低了参数冗余，使得模型在相同的激活参数量下，取得更好的性能。而在此基础上，更细粒度的专家划分，亦能够持续提升模型性能。
...
Softmax将多个专家的得分处理为互斥的概率分布，其总和恒为1，强制模型在类别间做出“单选”；而Sigmoid为每个专家独立地输出一个介于0到1之间的得分，允许多个专家有较高的得分，天然支持“多选”场景。如图2-4所示，一组专家*A*、*B*、*C*、*D*，在Sigmoid下得分为：0.95、0.88、0.36和0.53，专家*A*和*B*都有较高的得分，都可能被选中；而在Softmax下得分为：0.68、0.26、0.02和0.04，专家*A*得分突出，而专家*B*被抑制，不利于在细粒度MoE模型中进行Top-*K*多专家选择。

![](pic/2-4.png)

图 2-4 Sigmoid与Softmax在MoE路由中的比较
...
首先，模型计算输入 Token 的隐藏状态 $\text{u}_{\text{t}}$ 与第 $\text{i}$ 个路由专家的质心向量 $\text{e}_{\text{i}}$ 之间的相似度。DeepSeek-V3 使用 Sigmoid 函数作为激活函数来得出亲和度分数 $\text{s}_{\text{i}\text{,}\text{t}}$：

$$
\text{s}_{\text{i}\text{,}\text{t}}\text{=Sigmoid(}\text{u}_{\text{t}}^{\text{T}}\text{e}_{\text{i}}\text{)}
$$

这一设计使得每个专家对 Token 的亲和度评估在 $\text{(0,1)}$ 区间内独立进行，而非像 Softmax 那样在所有专家间进行强制的概率竞争分布。

（2）Top-$\text{K}$ 专家选择

不同于传统MoE路由选择为数不多的1或2个专家，由于采用细粒度专家分割策略，DeepSeekMoE会选择亲和度最高的$\text{K}$个专家，即采用Top-$\text{K}$ 路由策略。通常，在DeepSeek-V3中，$\text{K}\text{=8}$。

对于选中的专家，其亲和度分数将被作为门控值参与后续计算；对于未被选中的专家其对应的门控值将被置为 0，不参与后续计算。

（3）门控值归一化

为了确保后续加权求和的数值稳定性，模型仅对被选中的$\text{K}$个专家的亲和度分数进行归一化处理，得到最终的门控值 $\text{g}_{\text{i}\text{,}\text{t}}$：

$$
\text{g}_{\text{i}\text{,}\text{t}}\text{=}\frac{\text{s}_{\text{i}\text{,}\text{t}}}{\sum_{\text{j}\text{∈Top-K}}^{\text{\!}}\text{s}_{\text{j}\text{,}\text{t}}}
$$
...
在训练过程中，为每个专家 $\text{i}$ 引入了一个偏置项 $\text{b}_{\text{i}}$,用于人为调整专家的排序。进行 Top-$\text{K}$ 路由选择时，将 Token 与专家之间的亲和度分数 $\text{s}_{\text{i}\text{,}\text{t}}$ 结合偏置项，依据 $\text{s}_{\text{i}\text{,}\text{t}}\text{+}\text{b}_{\text{i}}$ 的值进行排序，并筛选前$\text{K}$个。但对被选中的专家计算门控值（Gating Value，即路由权重）时，仍使用原始的归一化亲和度$\text{s}_{\text{i}\text{,}\text{t}}$。也就是说，偏置项 $\text{b}_{\text{i}}$ 仅参与专家筛选，但不参与权重计算。其路由决策的数学表达如下：

$$
{\text{g}^{\text{'}}}_{\text{i}\text{,}\text{t}}\text{=}\left\{ \begin{matrix}
\text{s}_{\text{i}\text{,}\text{t}}\text{,} & \text{如果 }\text{s}_{\text{i}\text{,}\text{t}}\text{+}\text{b}_{\text{i}}\text{ 在 Top-}\text{K}\text{ 中} \\
\text{0,} & 
\end{matrix} \right.\ 
$$
...
为了实现自适应均衡，在模型训练时监控每个批次内各专家的实际负载，在该批次训练结束时，系统根据负载统计更新偏置项 $\text{b}_{\text{i}}$：对于负载过重的专家，减小其偏置值；对于负载过轻的专家，则增加其偏置值。通过这种动态反馈调节，系统能够使专家负载收敛至平衡状态，同时避免了辅助损失对模型性能的负面影响。

图2-5显示的实验结果表明了细粒度专家划分和无辅助损失的负载均衡方案给专家的专业化分工带来的提升。该热力图显示了在处理英语维基百科、Github 代码和数学3个不同领域的数据时，专家的激活情况。可以看出，相比于传统采用辅助损失的模型，无辅助损失模型的专家激活模式差异更为明显，图中出现了明显的“条纹状”分布。这表明专家不再被迫“平摊”所有任务，而是能够根据内容特征实现高度的专业化分工（图2-5仅展示了第9层和第18层的情况，其他层次也有相似的实验结果）。

![](pic/2-5.png)

图2-5 有辅助损失和无辅助损失模型在Pile测试集中三个不同领域的  
专家负荷对比，颜色越深代表相对负荷越高
...
其中，$\text{f}_{\text{i}}$ 代表专家 $\text{i}$ 在当前序列中的被选频率，$\text{P}_{\text{i}}$ 代表该专家的平均路由概率。在训练配置中，该平衡损失的权重系数 $\text{α}$ 被设置为极小值（如 $\text{10}^{\text{−4}}$ 量级）。这种配置确保了该损失项仅在专家负载出现巨大异常时产生梯度修正信号，而在常规情况下不会对模型的主梯度更新造成实质性干扰。
...
DeepSeek-V3 的卓越性能不仅源于其高效的前馈网络，更得益于其对注意力机制的深度重构。作为 Transformer 架构中负责捕捉长距离依赖和上下文关联的核心组件，注意力机制的设计直接决定了模型的推理效率与上下文窗口的扩展能力。DeepSeek 在此领域的创新主要体现在两个方面：在通用架构上采用了MLA（Multi-Head Latent Attention，多头潜在注意力）以解决显存瓶颈问题，在长文本场景下引入了DSA（Sparse Attention，稀疏注意力）以优化计算复杂度。本节将对这两项技术进行深入剖析。

### 2.3.1 多头潜在注意力 MLA

MLA 是 DeepSeek-V2 中首次提出并被 DeepSeek-V3 沿用的核心架构组件。其设计初衷是为了在保证模型具备标准MHA（Multi-Head Attention，多头注意力）强大学习能力的同时，将推理阶段的键-值缓存显存占用降低至极低水平。

#### 1. 键-值缓存的显存瓶颈分析

在基于 Transformer 的大语言模型推理过程中，模型采用“按词元逐个生成”的模式。为了避免重复计算，系统需要将所有历史词元在各层生成的键和值张量存储在显存中，这被称为键-值缓存。

对于一个标准的MHA模型，键-值缓存的显存占用量$M_{KV}$可以通过以下公式估算：

$$
M_{KV} = 2 \times b \times L \times n_{h} \times d_{h} \times n_{layers} \times P_{bytes}
$$

其中，$b$为批处理大小（Batch Size），$L$为上下文长度，$n_{h}$为注意力头数，$d_{h}$为每个头的维度，$n_{layers}$为模型层数，$P_{bytes}$为每个参数的字节数（如FP16/BF16为2字节，键-值缓存量化后可能更低）。

随着模型参数规模的扩大（$n_{h}$和$d_{h}$增加）以及长上下文应用的需求增长（$L$激增），键-值缓存迅速成为推理系统的主要瓶颈：

1）**显存容量限制**：巨大的键-值缓存挤占了用于存储模型权重和激活值的显存，限制了系统能够支持的最大批处理大小和上下文长度，直接导致推理吞吐量下降。

2）**显存带宽瓶颈**：在生成每个词元时，GPU需要从显存中读取完整的键-值缓存数据。当$L$较长时，受限于显存带宽，数据搬运时间远超计算时间，导致计算单元（Tensor Core）闲置，严重拖慢生成速度。

为了缓解这一问题，业界此前普遍采用MQA或GQA。

MQA让所有的查询头共享同一对键-值头。这意味着无论模型有多少个查询头，键-值缓存只需要存储一份数据。这种做法将键-值缓存的显存占用降至MHA的约$1/n_{h}$，极大提升了推理吞吐量。然而，这种“多对一”的强制映射通常会造成较为明显的性能损失。GQA则是一种折中方案：它将查询头分成$g$组，组内的查询头共享同一对键-值头。GQA的显存占用介于MHA和MQA之间，大约是MHA的$g/n_{h}$。
...
通过上述机制，MLA彻底改变了键-值缓存的存储逻辑：

- 标准MHA：需要缓存$\text{2}\text{n}_{\text{h}}\text{d}_{\text{h}}$个元素完整的键值张量。

- MLA：仅需缓存$\text{d}_{\text{c}}$个元素的潜在向量$\text{c}_{\text{t}}^{\text{KV}}$，和少量位置编码相关参数。相关内容将在后续介绍。

这一设计使得MLA的推理显存占用不再与注意力头数$n_{h}$线性相关，从而解除了增加头数对推理成本的约束。例如，DeepSeek-V3的键-值缓存仅用相当于不到3个标准注意力头的存储空间，支撑了全部128个头的计算，相比标准MHA，理论上可节省约98.2%的键-值显存。

（4）查询向量的低秩压缩

除了对键-值进行压缩，DeepSeek在训练阶段也对查询向量应用了类似的低秩压缩策略。尽管这不会减少推理时的键-值缓存（因为查询向量不需要缓存），但它可以显著降低训练过程中的激活值显存占用。其计算公式如下：
...
通过引入低秩联合压缩，MLA成功地将键-值缓存的信息密度大幅提升。然而，这种压缩机制却会与Transformer架构中至关重要的旋转位置编码（RoPE）产生数学上的冲突。DeepSeek提出了被称为“解耦RoPE”的数学技巧，解决了这一难题。
...
$$
\mathbf{q}_{t,i}^{C} = \mathbf{W}_{i}^{UQ}\mathbf{c}_{t}^{Q},\quad\mathbf{k}_{j,i}^{C} = \mathbf{W}_{i}^{UK}\mathbf{c}_{j}^{KV}\quad\quad\text{(1)}
$$

$$
(\mathbf{q}_{t,i}^{C})^{T}\mathbf{k}_{j,i}^{C} = (\mathbf{W}_{i}^{UQ}\mathbf{c}_{t}^{Q})^{T}(\mathbf{W}_{i}^{UK}\mathbf{c}_{j}^{KV})\quad\quad\text{(2)}
$$

根据矩阵乘法的结合律，上式可重构为：

$$
(\mathbf{q}_{t,i}^{C})^{T}\mathbf{k}_{j,i}^{C} = \mathbf{c}_{t}^{Q,T}\underset{可预先计算的静态矩阵}{\underbrace{(\mathbf{W}_{i}^{UQ,T}\mathbf{W}_{i}^{UK})}}\mathbf{c}_{j}^{KV}\quad\quad\text{(3)}
$$
...
$$
\mathbf{q}_{t,i}^{T}\mathbf{k}_{j,i} = \underset{\text{矩阵吸收优化，低维计算}}{\underbrace{\mathbf{c}_{t}^{Q,T}{\widetilde{\mathbf{W}}}_{i}\mathbf{c}_{j}^{KV}}} + \underset{\text{需显式计算，但维度仅为 }d_{h}^{R}}{\underbrace{(\mathbf{q}_{t,i}^{R})^{T}\mathbf{k}_{j}^{R}}}\quad\quad\text{(4)}
$$
...
**权重预融合**：矩阵吸收在模型加载时静态完成，推理时无额外开销，且可直接兼容键-值 Cache量化（如FP8/INT8）进一步压缩显存。

下表更清晰地对比了标准 MHA、MQA、GQA 和 MLA 的开销：

| **架构类型** | **键-值缓存构成（单 Token）** | **元素总数** | **相比标准MHA显存占用** |
|----|----|----|----|
| 标准 MHA | $2 \times n_{h} \times d_{h}$（Key + Value） | $2 \times 128 \times 128 = 32,768$ | 100%（基准） |
| MQA | $2 \times d_{h}$ | $2 \times 128 = 256$ | ~0.78% |
| GQA（8组） | $2 \times g \times d_{h}$ | $2 \times 8 \times 128 = 2,048$ | ~6.25% |
| MLA（DeepSeek-V2/V3） | $1 \times d_{c} + 1 \times d_{h}^{R}$ | $512 + 64 = 576$ | ~1.76% |

### 2.3.2 DeepSeek 稀疏注意力 DSA

通过引入 MLA 机制，DeepSeek 将推理过程中的 键-值 缓存显存占用降到了极低水平，彻底打破了长上下文场景下的显存瓶颈（Memory Bottleneck）。然而，解决了‘存得下’的问题后，DeepSeek 又面临着另一个严峻挑战：随着序列长度的不断攀升（如扩展至 128K 甚至更长），全注意力机制固有的平方级计算复杂度使得推理延迟急剧增加。为了在保持低显存优势的同时，进一步攻克长文本推理的计算瓶颈，DeepSeek-V3.2 在 MLA 的架构基础上，引入了 DSA（DeepSeek Sparse Attention）。

#### 1. 从全注意力到动态稀疏注意力的范式转变

在 MLA 成功将键值缓存（键-值 Cache）的显存占用压缩至极低水平之后，DeepSeek 面临的新挑战来自计算复杂度本身。随着大语言模型上下文窗口从 4K、32K 扩展至 128K 甚至更长，标准注意力机制 $O(L^{2})$ 的计算复杂度已成为制约模型效率的核心瓶颈。
...
DeepSeek 提出的**DeepSeek Sparse Attention（DSA）**基于一个关键观察：并非所有历史词元都对当前查询同等重要。对于任意给定的查询，真正需要精细关注的信息通常集中在少数关键位置——这些位置可能分散在文本的不同区域，而非连续聚集。因此，理想的注意力机制应当具备动态筛选能力：先快速识别出与当前查询最相关的候选词元，再对这些精选的少量词元执行完整的注意力计算。
...
**第一阶段：粗粒度筛选——闪电索引器（Lightning Indexer）**  
...
在 DeepSeek-V3.2 的架构中，DSA 被实例化于 MLA 的 MQA（Multi-Query Attention）模式之上。这意味着每个潜在向量（Latent Vector）作为键值入口，可被多个查询共享，进一步提升了稀疏注意力的计算效率。论文显示，这种设计使得 DSA 在保持与 Dense Attention 相当性能的同时，在 128K 上下文场景下实现了数倍的端到端加速。
...
#### 2. 两阶段流水线（一）粗粒度筛选：闪电索引器 (Lightning Indexer) 与块级筛选
...
$$
(\mathbf{q}_{t,i}^{C})^{T}\mathbf{k}_{j,i}^{C} = \mathbf{c}_{t}^{Q,T}{\widetilde{\mathbf{W}}}_{i}\mathbf{c}_{j}^{KV},\quad j \in \mathcal{S}_{t}\quad\quad\text{(5)}
$$
...
更重要的是，这种复杂度降低是**端到端**的。由于仅需加载 $\mathbf{k}$ 个潜在向量而非全部 $L$ 个，显存带宽压力同步降低，配合 MLA 的显存优化，使得在 H800 GPU 上处理 128K 长文本的推理成本（如图 3 所示）从每百万 Token 0.7 美元（Prefilling）和 2.4 美元（Decoding）降至约 0.2 美元和 0.3 美元，实现了数量级的成本优化。
...
2)**索引器梯度**：索引器通过独立的 KL 散度损失 $\mathcal{L}^{I}$ 进行优化（见 2.3.2.2 节），该损失仅基于选中的集合 $\mathcal{S}_{t}$ 计算，确保索引器学会在 Top-K 约束下最大化召回关键信息。

这种解耦的优化策略避免了梯度估计的方差问题，使得 DSA 能够稳定训练 15,000 步（约 943.7B Token），逐步适应稀疏模式而不出现性能崩溃。

#### 4. DSA 在超长上下文 (128K+) 场景下的计算复杂度分析
...
- **计算量减少 98.4%**，且随着 $L$ 继续增长，优势进一步放大（见图 3 成本曲线）。

**（2）端到端成本构成与内存墙突破**

在实际推理系统中，成本不仅包括计算操作数（FLOPs），更受限于**显存带宽**（Memory Bandwidth）。长上下文推理的主要瓶颈在于 键-值 缓存的反复读取——在解码阶段，每生成一个新 Token，都需要从显存加载整个历史 键-值 Cache。

DSA 在此展现了与 MLA 的**协同增效**：

- **MLA 降低单次读取成本**：通过低秩压缩，每个 Token 的 键-值 缓存仅需 576 维（$d_{c} + d_{h}^{R}$），相比标准 MHA 的 32,768 维（$2 \times n_{h} \times d_{h}$），单次读取带宽降低 98.2%。
...
论文图 3 展示了在 H800 GPU 上的实际服务成本（按 2 美元/GPU·小时计）：
...
这种差异源于解码阶段的计算特性：密集注意力每次都需要加载完整的 键-值 Cache（随 $L$ 增长），而 DSA 始终只加载固定的 $\mathbf{k}$ 个条目，使得解码成本与上下文长度基本解耦。
...
本节将深入探讨 DeepSeek 深度推理能力（Deep Reasoning） 的构建过程。首先，我们将介绍 多词元预测（MTP） 训练目标，阐述其如何通过改进传统的”下一词元预测“任务来增强模型的规划能力；随后，重点剖析 DeepSeek-R1 的训练演进，解析组相对策略优化（GRPO）与纯强化学习如何激发长思维链（CoT）的涌现；最后，我们将讨论这种推理能力的 迁移与融合 策略，包括模型蒸馏及推理模式的动态集成，以揭示从通用模型向推理模型跃迁的技术路径。

### 2.4.1 多词元预测训练 (Multi-Token Prediction, MTP)
...
为解决上述局限，DeepSeek-V3 采用**多词元预测（Multi-Token Prediction, MTP）**作为辅助训练目标。该方法的核心思想在于：在保持自回归框架的前提下，将模型的预测范围从单一的下一词元扩展至未来的连续 $D$ 个词元。形式上，对于序列中的每个位置 $i$，模型不仅需要预测 $t_{i + 1}$，还需并行预测 $t_{i + 2},...,t_{i + D}$。通过引入深度为 $D$ 的预测任务，MTP 实现了训练信号的**稠密化**（Densification）：每个输入位置现在可生成 $D$ 个独立的训练样本，从而显著提升了梯度的信息密度与数据利用效率。

此外，扩展预测范围促使模型在当前位置生成表示时，必须同时考虑对未来多个词元的预测需求。这种机制强制模型学习**预规划（Pre-planning）**其内部表示，即当前层的隐状态需编码足够的信息以支持对未来 $D$ 个位置的有效预测。相较于传统单步预测仅关注immediate next token的局部优化，MTP 引导模型构建更具前瞻性的语义表示，从而在数学推理、代码生成等需要多步逻辑连贯性的任务中展现出更强的性能。在 DeepSeek-V3 的具体配置中，$D$ 被设定为 1，即在标准下一词元预测的基础上额外增加一个未来词元的预测任务，以此在计算开销与性能增益之间取得平衡。
...
$$
\mathbf{h}_{i}^{'k} = \mathbf{M}_{k}\lbrack\text{RMSNorm}(\mathbf{h}_{i}^{k - 1});\text{RMSNorm}(\text{Emb}(t_{i + k}))\rbrack\quad\quad\text{(1)}
$$
...
$$
\mathbf{h}_{1:T - k}^{k} = \text{TRM}_{k}(\mathbf{h}_{1:T - k}^{'k})\quad\quad\text{(2)}
$$
...
$$
\mathcal{L}_{\text{MTP}}^{k} = - \frac{1}{T}\sum_{i = 2 + k}^{T + 1}\log P_{i}^{k}\lbrack t_{i}\rbrack\quad\quad\text{(3)}
$$

其中 $t_{i}$ 为第 $i$ 个位置的真实词元，$P_{i}^{k}\lbrack t_{i}\rbrack$ 表示第 $\mathbf{k}$ 个 MTP 模块对该词元的预测概率。最终的 MTP 损失为各深度损失的加权平均：

$$
\mathcal{L}_{\text{MTP}} = \frac{\lambda}{D}\sum_{k = 1}^{D}\mathcal{L}_{\text{MTP}}^{k}\quad\quad\text{(4)}
$$

此处 $\lambda$ 为权重超参数，在 DeepSeek-V3 的训练过程中，前 10T tokens 设置为 0.3，后续 4.8T tokens 调整为 0.1。通过这种设计，每个输入位置在训练过程中产生 $D$ 个独立的监督信号，显著提升了梯度的信息密度与训练效率。
...
训练过程中，研究团队设计了一套基于规则的奖励系统，主要包含两类信号：一是准确性奖励（Accuracy Rewards），即通过规则验证最终答案的正确性（如数学问题的标准答案匹配、代码的测试用例通过情况）；二是格式奖励（Format Rewards），要求模型将思考过程置于特定标签（如`<think>`与`</think>`）之间，以确保输出结构的可解析性。值得注意的是，该阶段刻意避免使用过程奖励模型（Process Reward Model）或针对推理内容的神经奖励模型，以防止奖励 hacking 现象并降低系统复杂度。
...
组相对策略优化（Group Relative Policy Optimization, GRPO）是支撑 DeepSeek-R1 系列模型高效训练的算法基石。该算法由 DeepSeek-AI 团队在 DeepSeek-Math 项目中首次提出，专门针对大规模语言模型（LLM）的强化学习场景设计，旨在解决传统近端策略优化（Proximal Policy Optimization, PPO）在高参数量模型训练中的计算瓶颈与内存瓶颈问题。以下从算法动机、数学原理、实现细节及性能特征四个层面展开阐述。
...
$$
\mathcal{J}_{GRPO}(\theta) = \mathbb{E}_{q \sim P(Q),\{ o_{i}\}_{i = 1}^{G} \sim \pi_{\theta_{old}}(O|q)}\left\lbrack \frac{1}{G}\sum_{i = 1}^{G}\left( \min\left( r_{i}(\theta)A_{i},\text{clip}(r_{i}(\theta),1 - \varepsilon,1 + \varepsilon)A_{i} \right) - \beta\mathbb{D}_{KL}(\pi_{\theta} \parallel \pi_{ref}) \right) \right\rbrack\quad\quad\text{(5)}
$$
...
$$
A_{i} = \frac{r_{i} - \text{mean}(\{ r_{1},r_{2},\cdots,r_{G}\})}{\text{std}(\{ r_{1},r_{2},\cdots,r_{G}\})}\quad\quad\text{(6)}
$$
...
为防止强化学习过程中策略模型过度优化（Over-optimization）奖励信号而偏离原始语言模型分布（即奖励 hacking 或模式崩溃），GRPO 引入了基于 KL 散度的正则项。其具体计算采用以下无偏估计形式：

$$
\mathbb{D}_{KL}(\pi_{\theta} \parallel \pi_{ref}) = \frac{\pi_{ref}(o_{i}|q)}{\pi_{\theta}(o_{i}|q)} - log\frac{\pi_{ref}(o_{i}|q)}{\pi_{\theta}(o_{i}|q)} - 1\quad\quad\text{(7)}
$$
...
- PPO 的前向计算复杂度为 $\text{O(2}\text{B}\text{⋅}\text{L}\text{⋅}\text{N}\text{)}$（Policy 与 Critic 各一次），反向传播为 $\text{O(}\text{B}\text{⋅}\text{L}\text{⋅}\text{N}\text{)}$；

- GRPO 的前向计算复杂度为 $\text{O(}\text{B}\text{⋅}\text{G}\text{⋅}\text{L}\text{⋅}\text{N}\text{)}$（需生成 $\text{G}$ 个响应），但由于无需 Critic 网络，总参数量减少，实际显存占用降低，且可通过批量处理（将 $\text{G}$ 个响应作为批次维度并行处理）优化吞吐量。

在 DeepSeek-R1 的具体训练中，GRPO 的超参数配置如下：

- **组大小（Group Size）**：通常设置为 8 或 16，平衡估计方差与计算开销；

- **裁剪系数** $\text{ε}$：0.2，标准 PPO 配置；

- **KL 惩罚系数** $\text{β}$：0.04 至 0.1 之间动态调整，前期较小以鼓励探索，后期增大以稳定输出分布；

- **学习率**：采用余弦退火（Cosine Annealing）策略，初始值约为 $\text{1×}\text{10}^{\text{−6}}$ 量级，随训练步数衰减。
...
在系统架构层面，两种模式的关键差异体现在**推理深度控制机制**上。思维模式采用扩展的链式思考（Chain-of-Thought, CoT）路径，允许模型生成数万 tokens 的逐步推导过程；非思维模式则通过长度约束奖励（Length Penalty）与早期终止策略，迫使模型在数十个 tokens 内完成结论输出。这种差异并非通过独立的模型副本实现，而是依托于同一套 MoE（Mixture-of-Experts）架构下的动态路由与上下文管理策略。

**（2）思维模式的能力构建**

思维模式的能力基础源于大规模强化学习计算与专业化数据蒸馏。在专家蒸馏阶段，研究团队针对数学、编程、逻辑推理等六个专业领域，分别训练了专门的专家模型。与常规做法不同，DeepSeek-V3.2 为每个领域生成了两套训练数据：

- **长思维链数据**：包含详细的逐步推导、自我验证与错误修正轨迹，平均序列长度超过 20K tokens；
...
在混合强化学习阶段，思维模式采用 Group Relative Policy Optimization（GRPO）算法进行优化，但关键差异在于**长度约束奖励的动态调整**。标准版 DeepSeek-V3.2 施加了严格的长度惩罚，以平衡推理深度与计算成本；而在高计算变体 DeepSeek-V3.2-Speciale 中，团队放松了长度约束，允许模型生成更长的推理轨迹（平均 45K tokens），从而在 IMO 2025 与 IOI 2025 等国际奥林匹克竞赛中达到金牌水平。

GRPO 的目标函数在思维模式中进行了针对性修改：

$$
\mathcal{L}_{\text{GRPO}}^{\text{thinking}}\mathbb{= E}\left\lbrack \frac{1}{G}\sum_{i = 1}^{G}\frac{1}{|o_{i}|}\sum_{t = 1}^{|o_{i}|}\min\left( r_{i,t}{\widehat{A}}_{i,t},\text{clip}(r_{i,t},1 - \varepsilon,1 + \varepsilon){\widehat{A}}_{i,t} \right) - \beta\mathbb{D}_{\text{KL}}(\pi_{\theta} \parallel \pi_{\text{ref}}) \right\rbrack - \lambda \cdot \text{Length}(o_{i})\quad\quad\text{(8)}
$$
...
在数据层面，非思维模式通过从思维模式的专家模型中蒸馏**捷径响应**（Shortcut Responses）构建训练集。具体而言，对于同一批问题，团队使用思维模式生成多步推导，同时训练一个轻量级蒸馏模型提取其中的关键推理节点，形成"输入-结论"的直接映射对。这种蒸馏策略使得非思维模式在 MMLU-Pro、GPQA Diamond 等知识密集型任务上保持了 85.0% 与 82.4% 的准确率，同时输出长度控制在思维模式的 1/3 以内（平均 7K-8K tokens）。

在推理架构上，非思维模式充分利用了 DSA（DeepSeek Sparse Attention）的短序列优化特性。对于长度小于 32K 的输入，系统采用模拟 MHA（Multi-Head Attention）的掩码模式，进一步降低计算延迟。下表展示了两种模式在典型基准上的性能与效率对比：

**表2-8 DeepSeek-V3.2 双模式性能对比**
...
\*注：非思维模式在复杂数学/编程任务上需开启 thinking mode 才能达到可比性能
...
具体而言，当模型处于工具调用流程中时，系统会保留历史推理内容（Thinking History），仅在新的用户消息到达时重置。这种设计避免了传统 R1 类模型在每次工具调用后重新推导完整问题上下文导致的 Token 冗余。如图 4 所示，在多轮工具调用中，思维模式生成的中间推理（Thinking 1.1、Thinking 1.2）被保留在上下文窗口中，直到用户输入新查询（Turn 2.1）时才被清除。

对于需要严格上下文管理的框架（如 Roo Code 或 Terminus），系统推荐使用非思维模式，因为这些框架通过用户消息模拟工具交互，可能与思考保留机制产生冲突。

**（5）计算效率与部署优化**

在硬件部署层面，双模式共享相同的 DSA 推理内核，但通过差异化的 键-值-缓存管理策略优化显存占用。思维模式由于生成长序列，采用分段式 键-值-缓存压缩与重计算（Recomputation）策略；非思维模式则启用激进的 缓存复用，支持更高的并发吞吐量。
...
此类数据强调逐步推导的完整性与逻辑严密性，平均序列长度 15K-30K tokens。
...
- **清除条件**：当检测到新的用户输入（Turn 2.1）时，清除上一轮的所有推理痕迹，但保留工具调用历史记录。

如图 4 所示，在多轮工具交互（Turn 1.1 → 1.2 → 1.3）中，模型生成的 Thinking 1.1 会被保留至后续轮次，使模型能够基于之前的推导结论继续深入，而非重新理解问题。这种机制使得在 Terminal Bench 2.0 等长程代码 agent 任务中，模型能够维持一致的调试思路，而非在每次代码执行后"遗忘"之前的错误分析。

**（3）大规模智能体任务合成管道**

为突破真实世界 agent 数据稀缺性的瓶颈，团队构建了**1,827 个独立环境、85,000 余个复杂提示**的合成管道（Synthesis Pipeline），覆盖搜索、代码工程、代码解释与通用规划四大领域。
...
利用 DeepSeek-V3.2 自身构建的自动化环境配置 agent，处理 Python、Java、JavaScript 等 7 种编程语言的依赖安装与测试执行。仅当应用金标准补丁后，测试用例呈现"假阳性转真阳性"（F2P）且无回归（P2F=0）时，环境才被视为构建成功。最终构建了数万个可复现的软件工程环境，支持在 SWE-bench Verified 等基准上的端到端训练。
...
2)**问题构造 agent**：利用搜索工具以可配置的深度与广度探索实体，整合信息为 QA 对；

3)**答案生成 agent**：通过异构配置（不同检查点、系统提示）生成多样化候选回答；

4)**验证 agent**：具备搜索能力的验证器通过多轮检索验证答案正确性，仅保留" ground-truth 正确且所有候选均错误"的样本（用于强化学习的负面样本挖掘）。

此外，结合已有的 helpful RL 数据集，筛选出搜索工具能提供显著收益的实例，构建混合训练集。
...
对于难以从真实世界直接获取的长尾任务（如旅行规划、复杂资源调度），团队开发了**自动环境合成 agent**， workflow 如下：

1)**数据库构建**：给定任务类别（如"杭州三日游规划"），agent 使用 bash 与搜索工具从互联网抓取城市、酒店、景点、餐厅数据，存入沙盒数据库；
...
通过此流程，团队生成了 1,827 个环境、4,417 个任务，经 RL 训练后保留非零 Pass@100 的实例。表 5 显示，这些合成任务对前沿闭源模型（GPT-5、Gemini-3.0 Pro）亦构成挑战（Pass@1 仅 34%-62%），证明了任务的难度与有效性。

**（4）强化学习中的能力涌现**

DeepSeek-V3.2 的 RL 阶段采用**单阶段混合训练**（Single-Stage Mixed RL），将推理、智能体与人类对齐数据合并训练，避免多阶段训练中的灾难性遗忘。奖励模型采用分层设计：

- **可验证任务**（数学、代码、工具调用）：使用规则-based 结果奖励（Rule-based Outcome Reward）+ 长度惩罚 + 语言一致性奖励；

- **开放任务**（通用 agent、写作）：使用生成式奖励模型（Generative RM），每个提示配备独立评分细则（Rubric），由模型自评生成奖励信号。

在智能体任务中，特别引入了**工具执行反馈奖励**：当模型生成的工具调用返回有效结果（如代码执行成功、搜索返回相关文档）时，给予中间步骤奖励，缓解稀疏奖励问题。

为验证合成数据训练后的泛化能力，团队在未见过的基准（Tau2Bench、MCP-Mark、MCP-Universe）上进行了消融实验。如图 5 所示，仅在合成通用 agent 数据上进行 RL 的模型（蓝色曲线），其性能从初始的 0.5 左右（DeepSeek-V3.2-SFT 基线）持续提升至 0.8 以上，显著优于仅在代码与搜索环境训练的模型（DeepSeek-V3.2-Exp，黑色虚线）。
...
在 128K 上下文限制下，长程 agent 任务常因 Token 耗尽而中断推理。DeepSeek-V3.2 引入了**动态上下文压缩策略**：
...
通过上述架构创新，DeepSeek-V3.2 成功将深度推理能力转化为可执行的工具调用智能，在保持开源模型成本优势的同时，首次在 SWE-bench、Terminal Bench 等硬核 agent 基准上逼近闭源前沿模型，为构建通用自主智能体（General-Purpose Autonomous Agents）奠定了技术基础。

## 2.5 CSA/HCA：混合压缩注意力与 KV Cache 的序列压缩

### 2.5.1 研究背景：注意力机制的演化简史

#### 2.5.1.1 问题的本质——KV Cache 的存储与计算瓶颈
...
其中上标 $a$ 表示"当前块"角色，上标 $b$ 表示"重叠到下一块"的角色（详见 2.5.2.2 节）。$\mathbf{W}^{aKV}, \mathbf{W}^{bKV}, \mathbf{W}^{aZ}, \mathbf{W}^{bZ} \in \mathbb{R}^{d \times c}$，即压缩权重不是单个标量，而是按 KV entry 的 $c$ 个维度分别给权重。
...
#### 2.5.2.3 Lightning Indexer：FP4 加速的 Top-k 选择
...
4. **输出**：Grouped Output Projection（见 2.5.6.4 节）
...
#### 2.5.6.2 RMSNorm on Q and KV Entries
...
#### 2.5.6.4 Grouped Output Projection
...
表2-11 50K 上下文、61 层架构的 KV cache 总量
...
### 2.5.8 从 NSA 到 CSA/HCA 的工程改造总结

表2-12 NSA 论文 vs V4 CSA/HCA 的工程对比
...
#### 2.6.1.2 超连接（Hyper-Connections）架构
...
表2-13 不同扩展率下的 HC 性能（OLMo-1B, 500B tokens）
...
表2-14 静态 HC vs 动态 HC（OLMo-1B, $n=4$, 500B tokens）
...
表2-15 HC 在 7B 参数规模的性能（OLMo-7B, DHC×4）
...
表2-16 HC 在 MoE 模型的性能（OLMoE-1B-7B, DHC×4）
...
#### 2.6.4.4 与 Layer Normalization 的互补关系
...
#### 2.7.4.4 Newton-Schulz 迭代：高效近似正交化
...
### 2.7.5 DeepSeek V4 的 Muon 实现

#### 2.7.5.1 完整算法（Algorithm 1）
...
#### 2.7.5.2 Hybrid Newton-Schulz 迭代细节
...
7. Gloeckle, F., et al. (2024). Better & Faster Large Language Models via Multi-token Prediction. arXiv:2404.19737.
...
11. DeepSeek-AI. (2025). DeepSeek-V3.2 Technical Report.
12. Yuan, J., et al. (2025). Native Sparse Attention: Hardware-Aligned and Natively Trainable Sparse Attention. arXiv:2502.11089.
13. Zhu, D., et al. (2025). Hyper-Connections. ICLR 2025. arXiv:2409.19606.
14. Xie, Z., et al. (2026). mHC: Manifold-Constrained Hyper-Connections. arXiv:2512.24880.
15. Keller Jordan, et al. (2024). Muon: An Optimizer for Hidden Layers in Neural Networks. https://github.com/KellerJordan/Muon
16. DeepSeek-AI. (2026). DeepSeek-V4: Towards Highly Efficient Million-Token Context Intelligence. Technical Report. https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/resolve/main/DeepSeek_V4.pdf
...
19. Sinkhorn, R. (1964). A Relationship Between Arbitrary Positive Matrices and Doubly Stochastic Matrices. The Annals of Mathematical Statistics, 35(2).
20. Sinkhorn, R., & Knopp, P. (1967). Concerning Nonnegative Matrices and Doubly Stochastic Matrices. Pacific Journal of Mathematics, 21(2).
21. Robbins, H., & Monro, S. (1951). A Stochastic Approximation Method. Annals of Mathematical Statistics, 22(3).
22. Polyak, B. T. (1964). Some Methods of Speeding up the Convergence of Iteration Methods. USSR Computational Mathematics and Mathematical Physics, 4(5).
23. Nesterov, Y. (1983). A Method for Unconstrained Convex Minimization Problem with the Rate of Convergence O(1/k^2). Doklady AN USSR, 269.
24. Schulz, G. (1933). Iterative Berechnung der reziproken Matrix. ZAMM, 13(1).
25. Kingma, D. P., & Ba, J. (2014). Adam: A Method for Stochastic Optimization. ICLR 2015. arXiv:1412.6980.
26. Loshchilov, I., & Hutter, F. (2017). Decoupled Weight Decay Regularization. ICLR 2019. arXiv:1711.05101.
