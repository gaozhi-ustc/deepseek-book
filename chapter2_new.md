...
本章将系统剖析 DeepSeek 模型家族的核心架构原理。全章以技术演进为主线，首先阐述DeepSeek LLM效率和智能“双主线”上升的发展路径；随后深入解析 其核心网络架构与注意力机制的设计细节；最后揭示以纯强化学习（Pure RL）为基础的推理能力构建，以及推理能力（深度思考模式）与通用能力（非思考模式）融合的机制。在此基础上，本章最后四节（2.5～2.8）将聚焦 2026 年发布的 DeepSeek-V4，剖析其面向百万级上下文的 CSA/HCA 混合稀疏注意力、mHC 流形约束超连接与 Muon 优化器等核心架构创新。
...
本章后续将对上述演进中涉及的核心组件进行深入剖析：2.2～2.4 节聚焦 V3/R1 体系的 DeepSeekMoE、MLA、DSA 与 GRPO 等技术；2.5～2.8 节则系统解析 V4 引入的架构创新，包括 CSA/HCA 混合稀疏注意力、mHC 流形约束超连接与 Muon 优化器。
...
### 2.5.4 MoE 参数演进：从 V3 到 V4

V4 对 MoE 层进行了多项重要调整，核心方向是"更多专家、更稀疏激活、更稳定路由"。

表2-7 DeepSeek MoE 参数变化（V3 vs V4-Pro vs V4-Flash）
...
#### 2.5.5.4 Anticipatory Routing（预防性路由）
...
## 2.6 CSA/HCA：混合压缩注意力与 KV Cache 的序列压缩

### 2.6.1 研究背景：注意力机制的演化简史

#### 2.6.1.1 问题的本质——KV Cache 的存储与计算瓶颈
...
其中上标 $a$ 表示"当前块"角色，上标 $b$ 表示"重叠到下一块"的角色（详见 2.6.2.2 节）。$\mathbf{W}^{aKV}, \mathbf{W}^{bKV}, \mathbf{W}^{aZ}, \mathbf{W}^{bZ} \in \mathbb{R}^{d \times c}$，即压缩权重不是单个标量，而是按 KV entry 的 $c$ 个维度分别给权重。
...
#### 2.6.2.3 Lightning Indexer：FP4 加速的 Top-k 选择
...
4. **输出**：Grouped Output Projection（见 2.6.6.4 节）
...
#### 2.6.6.2 RMSNorm on Q and KV Entries
...
#### 2.6.6.4 Grouped Output Projection
...
#### 2.7.1.2 超连接（Hyper-Connections）架构
...
#### 2.7.4.4 与 Layer Normalization 的互补关系
...
#### 2.8.4.4 Newton-Schulz 迭代：高效近似正交化
...
### 2.8.5 DeepSeek V4 的 Muon 实现

#### 2.8.5.1 完整算法（Algorithm 1）
...
#### 2.8.5.2 Hybrid Newton-Schulz 迭代细节
